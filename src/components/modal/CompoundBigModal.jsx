...
function CompoundBigModal({open, pendingTx, ...props}) {
  ...
  useEffect(() => {
    setActionName(shouldApprove()?"Approve Benkei" : shouldApproveLp()? "Approve Lp":"Compound")
  ...
  return (
    <>
      ...
      <CaelumPaper className="staking-modal flex flex-col">
        ...
        {actionName==="Approve Benkei" && <p className='w-full text-center'>{`You need to approve ${getFormattedDisplayNumber(inputValue*data.benkeiRateForEth*2)} BENKEI to compound`}</p>}
